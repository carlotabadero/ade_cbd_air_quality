...
particle <- mutate(particle, date_time = dmy_hm(particle$date_time, tz = "Australia/Adelaide"), year = year(date_time), month = month(date_time), wday = wday(date_time),hour = hour(date_time))

# Assign proper month levels and orders
particle <- mutate(particle, month = factor(particle$month, levels = 1:12, labels = month.abb))
...
season_match <- data.frame(month = 1:12,
...
particle <- particle %>% 
  mutate(season = fct_reorder(season, month))
```
```{r}
...
by_season <- by_season %>% 
  mutate(perc_delta_pm10 = (avg_pm10_season - avg_pm10)/avg_pm10,
         perc_delta_pm2.5 = (avg_pm2.5_season - avg_pm2.5)/avg_pm2.5)
